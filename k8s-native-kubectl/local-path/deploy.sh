...
# TDengine K3s 部署脚本（纯 kubectl 版本 - local-path 自动 PV 方案）
# 使用 kubectl apply -f 直接部署，无需 Kustomize
# 依赖: local-path StorageClass 自动分配 PV
...
echo "========================================"
echo "TDengine K3s 部署脚本（纯 kubectl - local-path 自动 PV）"
echo "========================================"
...
# 2. 检查 StorageClass（local-path 自动分配 PV）
echo "[2/5] 检查 StorageClass..."
kubectl get sc local-path > /dev/null 2>&1 || {
    echo "错误: local-path StorageClass 不存在"
    exit 1
}
...
kubectl apply -f ${SCRIPT_DIR}/tdengine.yaml
