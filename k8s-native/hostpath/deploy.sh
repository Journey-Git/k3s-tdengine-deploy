...
# 通过 kubectl 在节点上创建目录
echo "  检查并创建宿主机目录..."
kubectl get pod -n kube-system -l app=local-path-provisioner -o wide > /dev/null 2>&1
if [ $? -eq 0 ]; then
    # 尝试通过 local-path-provisioner pod 创建目录
    LOCAL_PATH_POD=$(kubectl get pod -n kube-system -l app=local-path-provisioner -o jsonpath='{.items[0].metadata.name}')
